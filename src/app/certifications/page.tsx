...
function certIcon(name: string) {
  ...
  if (n.includes("leapwork") || n.includes("accelq")) return "fa-solid fa-robot";
...
export default function CertificationsPage() {
  return (
    <Section
      ...
      title={<>Certifications <span className="gradientText">&amp; Credentials</span></>}
      subtitle="Professional certifications and technical training completed to support quality engineering and cloud infrastructure work."
    >
          ...
          <div key={c.name} className="card" style={{ display: "flex", flexDirection: "column" }}>
            <div style={{ flex: 1 }}>
              <div className="certRow">
                ...
                <span className={`badge ${c.status === "verified" ? "badgeVerified" : "badgeProgress"}`}>
                  {c.status === "verified" ? "Verified" : "In Progress"}
                </span>
              </div>

              <h3 className="h2" style={{ fontSize: "1rem", marginTop: 14, marginBottom: 2 }}>
                {c.name}
              </h3>
              {c.issuer && (
              ...
              {(c.date || c.targetDate) && (
                <p className="muted" style={{ marginTop: 12 }}>
                  ...
                  {c.status === "verified" ? `Completed: ${c.date}` : c.targetDate}
                </p>
              )}
              {c.credentialId && (
            ...
            {c.attachment ? (
              ...
              <div className="btnGhost" style={{ marginTop: 16, width: "fit-content", opacity: 0.6, cursor: "default" }}>
                Awaiting completion
              </div>
            )}
          </div>
