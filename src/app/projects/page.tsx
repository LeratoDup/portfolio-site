      ...
      <div className="stack">
        {projects.map((p) => (
          <div key={p.slug} className="card">
            <div className="stack">
              <div>
                <h3 className="h2" style={{ margin: 0 }}>{p.title}</h3>
                <p className="muted" style={{ marginTop: 8 }}>{p.oneLiner}</p>
              </div>
              ...
              <div className="grid3" style={{ alignItems: "start" }}>
                <div>
                  <p className="muted" style={{ margin: 0, fontWeight: 600 }}>Tech stack</p>
                  <ul className="list">
                    {p.stack.map((s) => (
                      <li key={s}>{s}</li>
                    ))}
                ...
                <div>
                  <p className="muted" style={{ margin: 0, fontWeight: 600 }}>What I built</p>
                  <ul className="list">
                    {p.highlights.map((h) => (
                      <li key={h}>{h}</li>
                    ))}
                ...
                <div>
                  <p className="muted" style={{ margin: 0, fontWeight: 600 }}>CI/CD and outcomes</p>
                  <ul className="list">
                    {p.ciCd.map((c) => (
                      <li key={c}>{c}</li>
                    ))}
